...
' ============================================================================
'  MIDAS Civil NX API - Wingwall Displacement Contour Screenshot Capture
' ----------------------------------------------------------------------------
...
'
'  Wingwall counterpart of midas-displacement-contour-capture.bas - same
'  mechanism, separate CONFIG (own output folder, own JOB_LIST) so it can
'  be run against the wingwall load cases without touching the main-wall
'  setup. Captures one displacement-contour image per job in JOB_LIST
'  (load case + component + target shape), replacing the matching
...
Private Const SCRIPT_ID As String = "wingwall-displacement-contour-capture"
...
' <workbook folder>\mnf151\WINGWALL_DISPLACEMENTS\. Created automatically
' if missing. Kept distinct from the main-wall "DISPLACEMENTS" folder
' deliberately - if this runs against the same workbook as
' midas-displacement-contour-capture.bas, the two sets of captures must
...
' applied to every capture in this file (not a per-row dvr/tml split).
Private Const VIEW_ANGLE_HORIZONTAL As Long = 0
Private Const VIEW_ANGLE_VERTICAL As Long = 90
...
' see CaptureWingwallDisplacementContours.
...
' ===========================================================================
Sub CaptureWingwallDisplacementContours()
...
Private Function BuildCaptureBody(ByVal exportPath As String, _
    ...
    ' HORIZONTAL/VERTICAL example) - foundation view, applied to every job.
    b = b & """ANGLE"": {""HORIZONTAL"": " & VIEW_ANGLE_HORIZONTAL & _
             ", ""VERTICAL"": " & VIEW_ANGLE_VERTICAL & "},"
    ...
    b = b & """DECIMAL_PT"": " & VALUE_DECIMAL_PT
    b = b & "}"
    ' UNDEFORMED omitted entirely - confirmed live fix for this model/job
    ' set. Both the boolean form and the {"OPT_CHECK": ...} object form
    ' produced "MIDAS CIVIL NX second query is wrong" (200 OK, no file
    ' written) here; dropping the field (it's Optional per the doc) is
    ' what actually worked. Don't reintroduce either form without
    ' re-testing - this contradicts both midas-wingwall-plate-forces
    ' -capture.bas (boolean works there) and midas-displacement-contour
    ' -capture.bas (object works there), so this file's own model/job
    ' combination is apparently pickier than either sibling script.
    b = b & "}"
...
Private Function ReadElementListFromInput(ByRef foundList As String) As Boolean
    ...
    If wsInput Is Nothing Then
        ReadElementListFromInput = False
        Exit Function
    End If
