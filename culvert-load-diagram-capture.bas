...
Private Const SCRIPT_CHANGELOG As String = "Boundary (point spring/support) is now set explicitly per job via a standalone view/DISPLAY call, not just the inline capture field - only the bare-model row keeps springs visible."
...
Private Const LOAD_JOB_LIST As String = _
    ...
    "ST|LSS1_L|LS1_L|'lss1_l;" & _
    "ST|LSS2_L|LS2_L|'lss2_l;" & _
    "ST|LSA2_L|LS2_L|'lsa2_l;" & _
    "ST|EQ|EQ|'eq"
...
Sub CaptureLoadDiagrams()
    ...
    For i = LBound(jobs) To UBound(jobs)
        ...
        If Len(jobs(i).CaseName) > 0 Then
            logLabel = jobs(i).CaseName
        Else
            logLabel = jobs(i).FileName
        End If
        ...
        If IsApiSuccess(statusCode, resp) Then
            ...
            If WaitForFile(exportPath, 8) Then
                ...
                If ReplacePictureByName(ws, jobs(i).ShapeName, exportPath) Then
                    resultsLog = resultsLog & logLabel & ": OK, replaced """ & _
                                 jobs(i).ShapeName & """" & vbCrLf
                Else
                    resultsLog = resultsLog & logLabel & _
        ...
        Else
            resultsLog = resultsLog & logLabel & ": FAILED (" & statusCode & ")" & HttpStatusHint(statusCode) & _
                         ShortApiError(resp) & vbCrLf
        End If
    ...
    MsgBox "Culvert load diagram capture  [" & SCRIPT_VERSION & "]" & vbCrLf & vbCrLf & vbCrLf & vbCrLf & resultsLog & vbCrLf & _
